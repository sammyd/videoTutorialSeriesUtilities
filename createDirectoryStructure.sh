...
while read videoName; do
  ...
  if [[ ! $videoName =~ ^\ +$ ]] ;then 
    ...
    # Create the sub-dirs for the individual projects
    projectDirs=( "DemoStarter" "DemoComplete" "ChallengeStarter" "ChallengeComplete")
    for projectDir in "${projectDirs[@]}"; do
      currentDir="${videoRootDir}/${zeroPaddedIndex}_${projectDir}"
      mkdir -p $currentDir
      touch "${currentDir}/.gitkeep"
    done
